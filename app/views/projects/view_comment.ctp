<?php 
if($session->read('Auth.User.group_id')!='7' && $session->read('Auth.User.group_id')!='6' && $session->read('Auth.User.group_id')!='5' && $session->read('Auth.User.group_id')!='2' && $session->read('Auth.User.group_id')!='1' && ($session->read('Auth.User.group_id')!='4' || ($session->read('Auth.User.username') != $project['Project']['created_by'])) && ($session->read('Auth.User.group_id')!='3' || ($session->read('Auth.User.username') != $project['User']['username'])))
{
...
<li><?php echo $this->Html->link(__('Edit Project', true), array('action' => 'edit', $project['Project']['id'])); ?> </li>
<li><?php echo $this->Html->link(__('List Users', true), array('controller' => 'users', 'action' => 'index')); ?> </li>
<li><?php echo $this->Html->link(__('Assign this project', true), array('action' => 'assign', $project['Project']['id']));  ?> </li>
<?php
}
