<?php
$divId = 'coment_add_'.$_model.'_'.$_project_id;
echo $ajax->div($divId); 
$session->flash();
?>
<?php
if ( !empty($successful) ) {
	echo $javascript->codeBlock($ajax->remoteFunction(array(
		'url' => '/coments/index/'.$_model.'/'.$_project_id,
		'update' => 'coments_'.$_model.'_'.$_project_id,
	)));
}
?>
...
<?php echo $ajax->form('add', 'post', array('model' => 'Coment','onsubmit' => 'fresh()','enctype' => 'multipart/form-data', 'update' => $divId));?>
		...
		echo $form->input('file', array('type' => 'file','name'=>'data[Attachment][file][]'));
?>
